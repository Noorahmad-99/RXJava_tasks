...
object RXJavaTasks {
    ...
    fun task1(): Observable<String> {
        return Observable.just("A B C D E F G H I G K L M N O P Q R S T U V W X Y Z").zipWith(Observable.interval(1, TimeUnit.SECONDS),
            {item, _ -> item.split(" ").toString() })
    }
    ...
    fun task5(): Observable<String> {
        ...
        return Observable.zip(firstObservable,secondObservable ,{ letter, number -> "$letter$number" })
    }
